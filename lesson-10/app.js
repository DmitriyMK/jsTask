...
const ARRAY_FIELDS = [
  { name: 'book_name', label: 'Название', price: 300 },
  { name: 'price', label: 'Цена', price: 50 },
  { name: 'author', label: 'автор', price: 80 },
  { name: 'country', label: 'страна', price: 390 },
  { name: 'url', label: 'картинка', price },
  {
    name: 'rate',
    label: 'рейтинг',
    choices: [1, 2, 3, 4, 5],
...
class InputField extends Field {
  render() {
    const html = `
                  <label class="label"><span>${this.label}</span>
...
class Book {
  ...
  render() {
    ...
    const html = `<div>Новая книга  - ${this.book.name} - ${price}</div>`
    return html;
  }
}
